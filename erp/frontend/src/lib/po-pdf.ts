...
export async function generatePoPdf(po: PurchaseOrder): Promise<void> {
  ...
  // PO Number / Revision / Date boxes
  const bx = rightBlock
  const by = y + 10
  const bw1 = 28, bw2 = 16, bw3 = 36
  ...
  doc.text("REVISION", bx + bw1 + 1, by + 3.5)
  doc.rect(bx + bw1 + bw2, by, bw3, 5)
  doc.text("DATE:", bx + bw1 + bw2 + 1, by + 2)
  doc.setFontSize(5)
  doc.text("AN/YR     MO     JR/DY", bx + bw1 + bw2 + 1, by + 4.5)

  // Values row
  doc.rect(bx, by + 5, bw1, 7)
  doc.rect(bx + bw1, by + 5, bw2, 7)
  doc.rect(bx + bw1 + bw2, by + 5, bw3, 7)

  doc.setFontSize(10)
  doc.setFont("helvetica", "bold")
  doc.text(po.po_number, bx + 2, by + 10)
  doc.setFontSize(9)
  doc.text(String(po.revision ?? 0).padStart(2, "0"), bx + bw1 + 4, by + 10)
  ...
  y += 4.5
  doc.setFontSize(9)
  doc.text(po.supplier?.name ?? "", m, y)
  y += 4
  doc.setFontSize(8)
  doc.setFont("helvetica", "normal")
  if (po.supplier?.address) {
    for (const line of po.supplier.address.split("\n")) {
      doc.text(line, m, y)
      y += 3.5
    }
  }
  ...
  // Attention
  if (po.supplier?.attention || po.supplier?.contact_name) {
    doc.setFontSize(7)
    doc.setFont("helvetica", "bold")
    doc.text("ATTENTION: ", m, y)
    doc.setFont("helvetica", "normal")
    doc.text(po.supplier?.attention ?? po.supplier?.contact_name ?? "", m + 22, y)
    y += 3.5
  }
  if (po.supplier?.phone) {
    doc.setFontSize(7)
    doc.text("TEL=" + po.supplier.phone, m, y)
    y += 3.5
  }
  if (po.supplier?.email) {
    doc.text(po.supplier.email, m, y)
    y += 3.5
  }
  ...
  doc.setFontSize(8)
  doc.setFont("helvetica", "normal")
  doc.text(po.ship_to ?? "ABOVE ADDRESS", midX, supplierStartY + 4.5)
  ...
  doc.text(po.requested_by ?? po.created_by ?? "", midX, supplierStartY + 16.5)
  ...
  const ed = po.expected_date ? new Date(po.expected_date) : null
  const tValues = [
    po.terms ?? "",
    po.fob ?? "FOB",
    po.requested_by ?? po.created_by ?? "",
  ...
  for (let li = 0; li < lines.length; li++) {
    const line = lines[li]
    const ipn = line.material?.internal_part_number ?? ""
    const mfg = line.manufacturer || line.material?.manufacturer || ""
    const mpn = line.manufacturer_pn || line.material?.manufacturer_pn || ""
    const desc = line.material?.description ?? ""
    const qty = parseFloat(String(line.quantity_ordered))
    const uc = parseFloat(String(line.unit_cost ?? 0))
    const total = qty * uc
  ...
  // ===== TOTAL =====
  y += 1
  const totalAmt = lines.reduce((s, l) => s + parseFloat(String(l.quantity_ordered)) * parseFloat(String(l.unit_cost ?? 0)), 0)
  ...
  doc.text(po.requested_by ?? po.created_by ?? "", m + 65, sigY + 5)
  doc.setFontSize(7)
  doc.setFont("helvetica", "normal")
